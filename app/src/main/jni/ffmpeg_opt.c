...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (o->nb_frame_sizes) {
        av_dict_set(&o->g->format_opts, "video_size",
                    (const char *) o->frame_sizes[o->nb_frame_sizes - 1].u.str, 0);
    }
...
static int read_ffserver_streams(OptionsContext *o, AVFormatContext *s, const char *filename)
{
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && !ost->stream_copy)
            ...
            choose_pixel_fmt(st, ost->enc_ctx, codec, (enum AVPixelFormat) st->codecpar->format);
       avcodec_parameters_from_context(ost->st->codecpar, ost->enc_ctx);
//        avcodec_copy_context(ost->enc_ctx, ost->enc_ctx);
        if (enc_config)
