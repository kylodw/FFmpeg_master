...
static int sub2video_get_blank_frame(InputStream *ist) {
    ...
    if ((ret = av_frame_get_buffer(frame, 32)) < 0)
        return ret;
    memset(frame->data[0], 0, (size_t) (frame->height * frame->linesize[0]));
    return 0;
}
...
//        av_image_fill_arrays()
       avpicture_fill(&pict, opkt.data, (enum AVPixelFormat) ost->st->codecpar->format,
...
static void set_encoder_id(OutputFile *of, OutputStream *ost) {
    ...
    if (!(format_flags & AVFMT_FLAG_BITEXACT) && !(codec_flags & AV_CODEC_FLAG_BITEXACT))
        av_strlcpy((char *) encoder_string, LIBAVCODEC_IDENT " ", (size_t) encoder_string_len);
    else
        av_strlcpy((char *) encoder_string, "Lavc ", (size_t) encoder_string_len);
    av_strlcat((char *) encoder_string, ost->enc->name, (size_t) encoder_string_len);
...
//        ret = avcodec_copy_context(ost->st->codec, ost->enc_ctx);
        ...
        if (ist) {
            ...
            for (i = 0; i < ist->st->nb_side_data; i++) {
                ...
                if (!dst)
                    return AVERROR(ENOMEM);
                memcpy(dst, sd->data, (size_t) sd->size);
        ...
        ost->enc_ctx->codec = ost->enc_ctx->codec;
