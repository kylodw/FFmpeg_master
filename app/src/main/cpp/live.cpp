...
void add_264_sequence_header(unsigned char *pps, unsigned char *sps, int pps_len, int sps_len) {
    int body_size = sps_len + pps_len + 16;  //16 H264的标准配置SPS,PPS ，需要16个字节的配置
    RTMPPacket *rtmpPacket = (RTMPPacket *) (malloc(sizeof(RTMPPacket)));
...
void *start_push(void *arg) {
    ...
    LOGE("%s", "进入RTMP_EnableWrite");
    if (!RTMP_Connect(rtmp, NULL)) {
        LOGE("%s", "RTMP_Connect连接失败");
        goto end;
    } else {
        LOGE("%s", "RTMP_Connect连接成功");
    }
    //拿到时间计时
    start_time = RTMP_GetTime();
    RTMP_ConnectStream(rtmp, 0);
    LOGE("RTMP_ConnectStream %d", RTMP_ConnectStream(rtmp, 0));
//    if (!) {
//        LOGE("%s", "RTMP_ConnectStream失败");
//        goto end;
//    }
    for (;;) {
        pthread_mutex_lock(&mutex);
